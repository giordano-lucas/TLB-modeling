...
int program_init(program_t* program){
	...
	program->nb_lines = 0;
	// correcteur: allocated needs to represent the amount of allocated memory in byte (see https://moodle.epfl.ch/mod/forum/discuss.php?d=16922)
	...
	// correcteur: the two following statements do the same thing, keep only one
	...
	// correcteur: useless since calloc already initializes the memory
...
int program_print(FILE* output, const program_t* program){
	M_REQUIRE_NON_NULL(program);
	M_REQUIRE_NON_NULL(output);
	// correcteur: missing validity check of program->listing
	
	for(int i = 0; i< program->nb_lines; i++){
		command_t com = program->listing[i];
		
		// correcteur: no need to check the validity of a command, you can assum they are valid in this function
...
int program_shrink(program_t* program){
	...
	// correcteur: cannot happen since unsigned
...
int program_add_command(program_t* program, const command_t* command){
	...
	// correcteur: needs to be adapted once program_init fix
...
int readCommand(FILE* input, command_t* command, int* validCommand){
	...
	*validCommand = 1; //command is supposed valid
	...
	// correcteur: missing test and propagation of error from handleRead and handleWrite
	switch (buffer[0]) {
        case 'R':
...
int handleRead(command_t* command, FILE* input){
	...
	// correcteur: missing error conversion check of strtoull (see [how to detect error](http://www.cplusplus.com/reference/cstdlib/strtoull/))
	...
	// correcteur: missing test and error propagation of init_virt_addr64
...
int program_read(const char* filename, program_t* program){
	...
	// correcteur: missing test and propagation of error code of program_init
	...
	while (!feof(file) && !ferror(file) ){
		...
		int validCommand = 0;
		// correcteur: missing test and propagation of error code of readCommand
		readCommand(file, &newC, &validCommand);

		if(validCommand)
			// correcteur: missing test and propagation of error code of program_add_command
			program_add_command(program, &newC);
		}
	// correcteur: missing check of ferror before closing the file since you may leave the loop because of ferror
	fclose(file);
	// correcteur: missing program shrinking
	return ERR_NONE;
	}
/**
...
int program_free(program_t* program){
	...
	// correcteur: missing finalisation of nb_lignes
	// correcteur: missing finalisation of allocated
	return ERR_NONE;
	}
