...
int init_phy_addr(phy_addr_t* paddr, uint32_t page_begin, uint32_t page_offset){
					   // FIXED : correcteur: missing validity check of page_begin  (hint: needs to be a multiple of something)
...
int print_physical_address(FILE* where, const phy_addr_t* paddr){
	M_REQUIRE_NON_NULL(paddr);
	// correcteur: you need to use another format specifier than PRIX32 for page_offset (hint: page_offset is a uint16_t)
